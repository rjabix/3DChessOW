...
public class ChessQueenMoveScript : ChessPieceBase
{
    public string team;
    ...
    private bool IsLegalMove(Vector3 Queen_position, Vector3 Cube_position)
    {
        if (!IsOnTheSameDiagonal(Queen_position, Cube_position) && !IsOnTheSameLine(Queen_position, Cube_position)) return false;
        if(IsOnTheSameDiagonal(Queen_position, Cube_position) && ! IsOnTheSameLine(Queen_position, Cube_position) && !IsPathBlockedDiagonal(Queen_position, Cube_position))
        {
        ...
        if(IsOnTheSameLine(Queen_position, Cube_position) && !IsOnTheSameDiagonal(Queen_position, Cube_position) && !IsPathBlockedForRook(Queen_position, Cube_position))
        {
            if (IsPositionOccupied(Cube_position))
            {
                Debug.Log("Position Occupied, getting trying to find it....");
    ...
    private bool IsOnTheSameLine(Vector3 Queen_Position, Vector3 Cube_Position)
    {
        int QueenRow = ChessCoordinates.PositionToRow(Queen_Position);
        char QueenFile = ChessCoordinates.PositionToFile(Queen_Position);
        int cubeRow = ChessCoordinates.PositionToRow(Cube_Position);
        char cubeFile = ChessCoordinates.PositionToFile(Cube_Position);

        if ((QueenRow == cubeRow && QueenFile != cubeFile) || (QueenRow != cubeRow && QueenFile == cubeFile))
            return true;

        return false;
    }

    private bool IsOnTheSameDiagonal(Vector3 Queen_Position, Vector3 Cube_Position)
    {
        int QueenRow = ChessCoordinates.PositionToRow(Queen_Position);
        char QueenFile = ChessCoordinates.PositionToFile(Queen_Position);
        int cubeRow = ChessCoordinates.PositionToRow(Cube_Position);
        ...
        int rowDiff = Mathf.Abs(cubeRow - QueenRow);
        int fileDiff = Mathf.Abs(cubeFile - QueenFile);
    ...
    private bool IsPathBlockedDiagonal(Vector3 start, Vector3 end)
    {
